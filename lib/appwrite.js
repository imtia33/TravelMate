...
import {
  ...
  export const appwriteConfig = {
    ...
    vehicleImageCollection:"67aeddb9002038b270f9",
    AdvertisementCOllection:"67b6fa89003d1b45a677",
  };
  ...
  export const sendMail = async (email,id) => {
    const recoveryUrl = "https://trav-x.web.app/verifying";
        try {
  ...
  export const getDistricts = async () => {   //gets the available districts
    try {
  ...
  export const getvehcileImage=async(vehicle)=>{
    try {
      const image = await databases.listDocuments(appwriteConfig.databaseId,appwriteConfig.vehicleImageCollection,[
  ...
  export async function getPlaceDetails(lat, lon) {
    ...
    try {
        ...
        const formattedData = {
            ...
            lon: String(place.geometry.coordinates[0]),
            area:place.properties.locality||"",
            street:place.properties.street || "",
            type:place.properties.osm_value || "",
        };
  ...
  export const getAdvertisements = async (district) => {
    const res= await databases.listDocuments(appwriteConfig.databaseId,appwriteConfig.AdvertisementCOllection,[
      Query.limit(5),
      Query.equal("District", district)
    ]);
    return res.documents;
  }
  export const getvisitingPlaces = async (district) => {

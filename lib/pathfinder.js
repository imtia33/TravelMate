...
const getClosestPolyline = async (extra, check, marker) => {
  ...
  for (const doc of extra) {
    ...
    if (close === null || res.distance < close.distance) {
      close = res;
      closeName = doc.Name;
      containingPolyline = doc.Coordinates;
    }
  }
  const closeLength = calculatePolylineLength(
    check
      ? containingPolyline.slice(close.index + 1, containingPolyline.length)
      : containingPolyline.slice(0, close.index + 1)
  );
  return {
    name: closeName,
    length: closeLength,

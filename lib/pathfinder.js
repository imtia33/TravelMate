...
export const fetchLocationDetails = async (name,district) => {  //currently has no use
  const db = await SQLite.openDatabaseAsync("routes.db");
  return await db.getAllAsync(
...
export const fetchLocationsInChunks = async (offset = 0, limit = 100, firstChar, district) => {
...
export const findBestMatch = async (search,district) => {
  ...
  while (offset < 1000) {
    const locations = await fetchLocationsInChunks(offset, 100, firstChar,district);
...
export const fetchLocations = async (district) => {
  const db = await SQLite.openDatabaseAsync("routes.db");
  return await db.getAllAsync("SELECT * FROM locations WHERE single = true AND District = ?", [district]);
};

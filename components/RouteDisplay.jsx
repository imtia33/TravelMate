  ...
  const renderRoutes = (subRoutes) => {
    ...
    const uniqueGroupedSteps = Object.values(groupedSteps).map((route) => {
      const uniqueVehicles = Array.from(
        new Set(route.vehicles.map((v) => v.vehicle))
      ).map((vehicle) => route.vehicles.find((v) => v.vehicle === vehicle));
      return { ...route, vehicles: uniqueVehicles };
    });
    const uniqueFromToValues = Array.from(
      new Set(subRoutes.flatMap((route) => [route.From, route.To]))
    );

    const mergedSteps = [];
    let previousStep = null;

    uniqueGroupedSteps.forEach((route, index) => {
      const hasWalk = route.vehicles.some((v) => v.vehicle === "Walk");

      if (
        previousStep &&
        hasWalk &&
        previousStep.vehicles.some((v) => v.vehicle === "Walk")
      ) {
        // Merge with the previous step
        previousStep.distance += route.distance; // Assuming distance is a property to sum
        previousStep.To = route.To;
      } else {
        mergedSteps.push(route);
        previousStep = route; 
      }
    });
    ...
    return (
      <View style={{ marginBottom: 75 }}>
        ...
        >
          ...
          >
            ...
            <View style={{ alignItems: "center" }}>
              ...
              <Text style={{ fontSize: 14, color: "#000", marginTop: 5 }}>
                {uniqueFromToValues.length} stops
              </Text>
            </View>
          </View>
        </View>
        ...
        >
          ...
          {mergedSteps.map((route, index) => {
            const hasWalk = route.vehicles.some((v) => v.vehicle === "Walk");
            const filteredVehicles = hasWalk
              ? route.vehicles.filter((v) => v.vehicle === "Walk")
              : route.vehicles;
            ...
            return (
              <View key={index} style={{ marginBottom: 20 }}>
                ...
                <ScrollView
                  ...
                  style={{ marginLeft: 25 }}
                >
                  {filteredVehicles.map((transportOption, optIndex) =>
                    transportOption.vehicle !== "Walk" ? (
                      ...
                      >
                        ...
                        >
                          <TouchableOpacity
                            onLongPress={() => {
                              showVehicleImage(transportOption.vehicle);
                            }}
                          >
                            <Image
                              source={
                                icons[getVehicleIcon(transportOption.vehicle)]
                              }
                        ...
                        <View style={{ marginLeft: 10, flex: 1 }}>
                          ...
                          >
                            ...
                            <Text style={{ fontSize: 20, color: "#F02E65" }}>
                              {" "}
                              •{" "}
                            </Text>
                      ...
                      >
                        ...
                        >
                          Walk {route.distance?.toFixed(1)} km
                        </Text>
                      </View>
                    )
                  )}
  ...
  return (
    ...
    >
      ...
      <ScrollView style={{ padding: 10 }}>
        {renderRoutes(
          routes[selectedRouteIndex][selectedSubRouteIndex] ||

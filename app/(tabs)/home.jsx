...
import { getAdvertisements, getvisitingPlaces, signOut } from '../../lib/appwrite';
import { getHealthcarePlaces } from '../../lib/healthcare';
...
const Home = () => {
  ...
  const handleLogout = async () => {
    try {
      await signOut();
      router.push('/sign-in'); // Redirect to home/sign-in page
    } catch (error) {
  ...
  const fetchHealthcarePlaces = async () => {
    try {
      const response = await getHealthcarePlaces(user?.bbox);
      setHealthcarePlaces(response);
      setLoading(false);
    } catch (error) {
  ...
  const handlePlaceSelect = (place) => {
    ...
    const formattedPlace = {
      ...
      Latitude: place.Lat || place.lat || place.Latitude,
      Longitude: place.Long || place.lon || place.Longitude,
      street: place.street || place.Location || ''
    };
